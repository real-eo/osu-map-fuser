...
// // #include "config.hpp"                                             // ORIGINAL CODE
...
{
    // // std::ifstream f(fname);                                       // ORIGINAL CODE
    std::ifstream f(fname.c_str(), std::ios::binary);                   // ! Fix      [real-eo]
    return f.good();
}
...
{
    if (stream) stop();
    // // std::wstring full_path = config::song_path + fname;           // ORIGINAL CODE
    std::wstring full_path = fname;                                     // ! Adjusted to use only fname for path     [real-eo]
